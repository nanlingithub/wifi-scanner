"""
网络概览标签页 - v1.4版本
特性：保留原有完整功能 + 集成优化的12等分雷达图
功能：WiFi扫描、信号强度显示、优化雷达图、实时监控、频段分析、信道优化、报告导出
        ...
        while self.monitoring:
            try:
                ...
                try:
                    ...
                    pass  # P3修复: 队列已满，忽略本次更新（正常行为）
                
                # 等待（根据速度）
                interval_str = self.interval_var.get()
                interval = int(interval_str.replace('秒', ''))
                time.sleep(min(interval, self.scan_interval / self.rotation_speed))
